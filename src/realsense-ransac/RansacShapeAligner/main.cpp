...
int main(int argc, char** argv)
{
    ...
    for (const auto& shape : shapes) {
        if (shape.first->Identifier() == 2) { // 2 equals "Cylinder" type
            ...
            // Inspired from CloudCompare plugins/core/Standard/qRANSAC_SD/src/qRANSAC_SD.cpp
            Vec3f G = cyl->Internal().AxisPosition();
            const auto& N = cyl->Internal().AxisDirection();
            auto angDir = cyl->Internal().AngularDirection();
            auto X = angDir;
            auto Y = N.cross(angDir);
            ...
            //std::cout << ... << std::endl; // std::endl = Inserts a newline character into the output sequence os and flushes it as if by calling os.put(os.widen('\n')) followed by os.flush()
            //std::cout.flush();
            std::cout << r << std::endl;
            std::cout.flush();
